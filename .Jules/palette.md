...
## 2025-01-20 - Tactile Feedback for Micro-interactions
**Learning:** For small interactive elements like a "Copy to clipboard" button inside a code block, visual feedback needs to be more than just an icon swap. Users benefit from clear textual feedback (e.g., "복사됨"), but also from tactile states. A subtle `:active` scale transform (e.g., `transform: scale(0.95)`) makes the button feel physically responsive. Furthermore, explicitly separating the `:focus-visible` state from a generic outline ensures keyboard users have distinct visual confirmation without compromising mouse user aesthetics.
**Action:** Always provide textual confirmation along with icon changes for non-obvious actions. Add tactile `:active` states for buttons and ensure `:focus-visible` outline styles are explicitly set.
